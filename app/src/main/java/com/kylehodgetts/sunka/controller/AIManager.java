...
public class AIManager extends EventHandler<GameState> {
    ...
    private final int PLAYER_AI = 1;
    private final int PLAYER_HUMAN = 0;
    ...
    public Tuple2<GameState, Boolean> handleEvent(Event event, final GameState state) {
        ...
        if (event instanceof NextTurn && state.getCurrentPlayerIndex() == myIndex) {
            ...
            timer.schedule(new TimerTask() {
                @Override
                public void run() {
                    bus.feedEvent(new PlayerChoseTray(performGreedyTurn(state), myIndex));
                }
            }, TIMERDELAY);
            Log.i(TAG, "End turn");
            Log.i(TAG, " ");
     ...
     */
    public int performGreedyTurn(GameState state) {
